...
namespace FAP
{
    ...
    struct Unit
    {
            ...
            if (unit.groundDamage || unit.airDamage)
            {
                switch (unit.unitType)
                {
                    case BWAPI::UnitTypes::Protoss_Carrier:
                        unit.groundDamage = BWAPI::UnitTypes::Protoss_Interceptor.groundWeapon().damageAmount()
                                            + BWAPI::UnitTypes::Protoss_Interceptor.groundWeapon().damageBonus() * upgradeLevel;
                        unit.airDamage = BWAPI::UnitTypes::Protoss_Interceptor.airWeapon().damageAmount()
                                         + BWAPI::UnitTypes::Protoss_Interceptor.airWeapon().damageBonus() * upgradeLevel;
                        break;
                    case BWAPI::UnitTypes::Terran_Bunker:
                        unit.groundDamage = BWAPI::UnitTypes::Terran_Marine.groundWeapon().damageAmount()
                                            + BWAPI::UnitTypes::Terran_Marine.groundWeapon().damageBonus() * upgradeLevel;
                        unit.airDamage = BWAPI::UnitTypes::Terran_Marine.airWeapon().damageAmount()
                                         + BWAPI::UnitTypes::Terran_Marine.airWeapon().damageBonus() * upgradeLevel;
                        break;
                    case BWAPI::UnitTypes::Protoss_Reaver:
                        unit.groundDamage = BWAPI::UnitTypes::Protoss_Scarab.groundWeapon().damageAmount()
                                            + BWAPI::UnitTypes::Protoss_Scarab.groundWeapon().damageBonus() * upgradeLevel;
                        break;
                    default:
        ...
        {
            static_assert(hasFlag(UnitValues::groundCooldown) && hasFlag(UnitValues::airCooldown),
                          "Set attack cooldown before setting attack speed upgrade");
        ...
        {
            static_assert(hasFlag(UnitValues::unitType) && hasFlag(UnitValues::groundDamage) && hasFlag(UnitValues::airDamage),
                          "Set unit type and damage before setting number of attackers");
