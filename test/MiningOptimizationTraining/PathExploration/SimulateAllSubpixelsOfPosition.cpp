                ...
                for (int x = 0; x < 256; x++)
                {
                    if (solutions[y * 256 + x] == 0)
                    {
                        ...
                        auto prepareResult = simWorker->prepareGatherPath(
                                BWAPI::PrepareGatherPathOptions(resendStartPos,
                                                                initialStateWithNoCannons.state).prepareReturnFrom(startPosition.patch->getBWIndex()));
                        if (!prepareResult)
                        {
                        ...
                        for (int resend = 20; resend < 50; resend++)
                        {
                            auto resendReturnResult = simWorker->simulateGatherPath(
                                    BWAPI::SimulateGatherPathOptions({resend}, prepareResult->state));
