#include "FullSaturationPathExplorationModule.h"

#include "Geo.h"

#include <random>

void FullSaturationPathExplorationModule::onFrame()
{
    InstrumentedDoNothingModule::onFrameStart();

    MiningTrainingModule::onFrame();

    if (initialize())
    {
        // TODO: Perform exploration logic
    }

    InstrumentedDoNothingModule::onFrameEnd();
}

bool FullSaturationPathExplorationModule::initialize()
{
    ...
    else if (BWAPI::Broodwar->getFrameCount() % 10000 == 23)
    {
        ...
        for (auto &worker : workers)
        {
            ...
            if (it != workerCreationOrderAndBase.end())
            {
                ...
                {
                    workerToAssignedPatch[worker] = *basePatches.rbegin();
                    CherryVis::log(worker->getID()) << "Assigned to patch @ " << BWAPI::WalkPosition(workerToAssignedPatch[worker]->center);
